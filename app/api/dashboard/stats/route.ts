  ...
  try {
    ...
    if (!supabaseUrl || !supabaseKey) {
      return NextResponse.json({
        ...
        error: true,
        message:
          "Variables de entorno de Supabase no configuradas. Agregue NEXT_PUBLIC_SUPABASE_URL y NEXT_PUBLIC_SUPABASE_ANON_KEY a .env.local",
        setupRequired: true,
      })
    }
    ...
    if (testError) {
      ...
      return NextResponse.json({
        ...
        ratings: { averageWorker: 0 },
        error: true,
        message: `Error de conexión a la base de datos: ${testError.message}. Verifique que las tablas existan ejecutando los scripts SQL.`,
        setupRequired: true,
      })
    }
